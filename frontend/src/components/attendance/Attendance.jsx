...
const Attendance = () => {
  ...
  const fetchAttendance = async () => {
    setLoading(true);
    try {
      const responnse = await axios.get("https://factory-flow-app-weld.vercel.app/api/attendance", {
        headers: {
  ...
  return (
    <div className="p-6">
      <div className="text-center">
        <h3 className="text-2xl font-bold">Gestion Des Présence</h3>
      </div>
      <div className="flex justify-between items-center mt-4">
        ...
        <p className="text-2xl">
          Marquer les employés pour <span className="font-bold underline">{new Date().toISOString().split("T")[0]}{" "}</span>
        </p>
        <Link
